...
def makeScalePyramid(im, targetPosition, in_side_scaled, out_side, avgChans):
    ...
    max_target_side = int(round(in_side_scaled))
    min_target_side = int(round(in_side_scaled))
    ...
    target_side = round(beta * in_side_scaled)
    tmp_region = getSubWinTracking(search_region, tmp_pos, (out_side, out_side), (target_side, target_side),avgChans)
...
def updata(score,area,pos,window):
    ...
    #这里的话，如果得到的是进行了缩放了的，就会对对目前得到的response进行惩罚
    #这里我没有进行缩放，为了让大家看到所以会进行窗口的惩罚
    #而对于多尺度而言，取中峰值最高的response 当做最好的scale
    responseMaps = score[:, :, :, 0]
    _area = 17 * 17
    # print('compare',_area,area)
    responseMap = cv2.resize(responseMaps[0,:,:],(_area,_area),interpolation=cv2.INTER_CUBIC)


    responseMap = responseMap - np.min(responseMap)
    responseMap = responseMap / np.sum(responseMap)

    responseMap = (1 - 0.5) * responseMap + 0.5 * window
    rMax,cMax = np.unravel_index(responseMap.argmax(),responseMap.shape)
    pCorr = np.array((rMax,cMax))
    dispInstanceFinal = pCorr - int(_area / 2)
    # dispInstanceInput = dispInstanceFinal * 8 / 16
    # dispInstanceFrame = dispInstanceFinal * area / 255
    dispInstanceFrame = dispInstanceFinal * area / 255
    newTargetPosition = pos + dispInstanceFrame
...
#所有的resize都是双三次插值
    ...
    with tf.variable_scope('siamese') as scope:
        scope.reuse_variables()
        scores = siam.build_target_layer(siam.target_holder)   #z
        # print(scores)
    ...
    while True:
        ...
        print(scale_search)
        scale_target = np.array(target_sz,np.float) * scales
        #搜索区域
        x_crop = makeScalePyramid(frame,pos,scale_search,255,avg)
        score = sess.run(scoreOp,feed_dict={siam.search_holder:np.expand_dims(x_crop,0)})

        new_pos = updata(score,round(area),pos,cos_window)

        pos = new_pos

        area = max(min_search_x,min(max_search_x,(1 - .59) * area + .59 * scale_search))

        target_sz = (1 - .59) * np.array(target_sz,np.float) + .59 * scale_target

        rect = pos - target_sz / 2
        p1 = tuple(np.round(rect).astype(int)[::-1])
        p2 = tuple(np.round(rect + target_sz).astype(int)[::-1])
